...
describe('Connection', () => {
    it('should have correctly assigned type', () => {
        ...
        const conn = new ThreadConnection<ThreadConfig>(channel.port1, ThreadConfig.A);
        expect(conn.threadType).toEqual(ThreadConfig.A);
    });
    it("should open connection", () => {
        const channel = new MessageChannel();
        const aToB = new ThreadConnection<ThreadConfig>(channel.port1, ThreadConfig.A);
        const bToA = new ThreadConnection<ThreadConfig>(channel.port2, ThreadConfig.B);
